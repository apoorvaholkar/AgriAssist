...
import './Box.css';
import { getSustainabilityInfo } from '../CommunityForm/infodata';
...
const Box = () => {
  ...
  const [sustainabilityData2, setSustainabilityData2] = useState([]); 

  useEffect(() => {

    const data = getSustainabilityInfo(); 
    setSustainabilityData2(data); 
    console.log(data); 
  }, []);

  const governmentSchemes = sustainabilityData2.filter(item => (
    item.title.startsWith('Government Scheme')
  ));

  const otherInfo = sustainabilityData2.filter(item => (
    !item.title.startsWith('Government Scheme')
  ));
